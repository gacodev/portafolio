        ...
        {projects.map((company) => (
          <VerticalTimelineElement
            ...
            iconStyle={{
              background: '#4A90E2',
              color: '#fff',
              width: 'clamp(32px, 8vw, 50px)', // Más pequeño para móviles
          ...
          >
            ...
            {company.projects.map((project) => (
              <div key={`${company.company}-${project.name}-${project.year}`} className="flex flex-col sm:flex-row items-start rounded-lg p-3 sm:p-4 lg:p-6 shadow-md transition-transform transform hover:scale-105 mb-3 sm:mb-4 bg-gradient-to-r from-gray-800/50 to-gray-900/50 backdrop-blur-sm w-full">
                <div className="flex-shrink-0 mb-3 sm:mb-4 sm:mr-4 lg:mr-6 w-full sm:w-auto flex justify-center sm:block">
                  {project.image ? (
                    <Image
                      src={project.image}
                      alt={project.name}
                      width={80}
                      height={80}
                      className="rounded-lg object-cover sm:w-[100px] sm:h-[100px]"
                    />
                  ) : (
                    <GravatarImage name={project.name} size={80} className="sm:w-[100px] sm:h-[100px]" />
                  )}
                </div>
